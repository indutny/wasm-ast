...
Parser.prototype.reserveGlobal = function reserveGlobal(name, type) {
  if (this.options.index !== true)
    return { type: 'Identifier', name: name };

  return this.globalScope.reserve(name, type);
};

Parser.prototype.fulfillGlobal = function fulfillGlobal(name, type) {
  if (this.options.index !== true)
    return { type: 'Identifier', name: name };

  return this.globalScope.fulfill(name, type);
};
...
Parser.prototype.parseTopLevelFn = function parseTopLevelFn(result) {
  ...
  var name = this.fulfillGlobal(this.expect('Identifier'), 'FunctionRef');
...
Parser.prototype.parseCall = function parseCall(name) {
  var fn = this.reserveGlobal(name, 'FunctionRef');
...
Parser.prototype.parseImport = function parseImport() {
  var names = [];
  do
    names.push({ type: 'Identifier', name: this.expect('Identifier') });
